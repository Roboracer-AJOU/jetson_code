...
다시 물어 영영 그 자리였다. 0.4 m 만 물러나면 같은 식이 50° 로 떨어져 그때부터
전진 탈출이 된다.
...
| 파라미터 | 기본 | 의미 |
| --- | --- | --- |
| `reverse_stuck_sec` | `1.0` | 이만큼 서 있으면 전진 실패로 본다 |
| `reverse_stuck_obstacle_m` | `0.79` | 이 안에 뭔가 있어야 "막혀서" 선 것 (범퍼 0.6 m) |
| `reverse_cooldown_sec` | `3.0` | 물러난 뒤 이만큼은 다시 안 건다 |
...
| `reverse_travel_m` | `0.40` | 이만큼 물러나면 끝내고 전진 재시도 |
| `reverse_max_sec` | `2.0` | 속도계가 0 이라 거리가 안 쌓여도 끊는다 |
| `reverse_min_clearance_m` | `0.60` | 뒤가 이만큼 안 비면 **시작하지 않는다** |
...
> 뒤에 있으면 그대로 민다. 그래서 후진 거리를 0.4 m 로 짧게, 속도도 기는
...
새로 드러나는 0.25~0.42 띠는 `wall_clearance_m`(0.12 → **0.20**) 로 넓힌
near-wall 가드가 맡는다. 그 띠 안의 클러스터는 `near_wall_min_points`(14)
와 `near_wall_min_span_m`(0.20) 를 넘어야 살아남는다 — 얇은 벽 잔차는
탈락하고 실물 박스는 근거리에서 60점 넘게 나오므로 통과한다.

> 벽 팽창을 줄이면 벽 잔차가 늘어난다. FGM 오작동이 다시 보이면 여기부터
> 의심할 것. M-of-N(4/6, 5.3.8) 과 near-wall 가드가 2차 방어선이다.
...
모자라면 회피를 포기하고 CSV 를 유지한다(`_mark_avoid_blocked`). 그러면 속도
정책이 감속을 걸고, 느려진 다음 주기에는 같은 경로도 통과한다 — **"박을 것
같으면 먼저 속도를 줄인다"** 가 이렇게 나온다. 끝까지 안 되면 AEB 가 받는다.

AEB 탈출 중에는 호출부가 `min_length_m` 을 따로 넘겨 짧은 경로를 허용하는데,
그때는 속도가 낮아 이 항이 거의 0 이라 서로 싸우지 않는다.

끄려면 `wall_stop_check_enable:=false`.
