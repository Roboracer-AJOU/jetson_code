...
| `reverse_half_width_m` | `0.25` | 후방 코리도 반폭 (전방보다 넉넉하게) |

후방 여유는 전방과 달리 **맵 필터를 안 쓴다.** 후진은 벽이든 장애물이든 닿으면
안 되는데, 아는 벽을 지우면 벽으로 밀고 들어간다. 라이다가 축보다 0.31 m 앞이라
뒤끝까지가 `LASER_TO_REAR_M` = 0.41 m 다 — 스캔 거리에서 이만큼 빼야 범퍼 여유다.
...
2. 현재 (s, d, d′) 에서 d → 0 으로 가는 quintic 을 생성.
   길이는 **속도·이탈량·합류각** 연동 (`_rejoin_length_for`):
   `L = clip(max(rejoin_time_sec × v_ego, √(5.77·|d0|·v² / rejoin_a_lat_mps2),
   1.875·|d0| / tan(ψ_max(v))), rejoin_min_length_m, rejoin_max_length_m)`
...
3. 그 경로도 `_truncate_path_at_collision` 을 통과해야 채택된다.
   막히거나 **코너에서 Frenet 이 접히면** `REJOIN` 을 포기하고 바로 `GLOBAL`
   — CSV 로 두는 편이 안전하다
...
**포기해야 하는 두 경우** — 둘 다 감속으로 해결되지 않는다:
